window.TRADINGTRIO_BUILD='2.5.5';
...
async function ttFetchFeed(f){const tries=[()=>ttText(f.url),()=>ttText('https://api.allorigins.win/raw?url='+encodeURIComponent(f.url)),()=>ttText('https://api.allorigins.win/get?url='+encodeURIComponent(f.url)).then(t=>JSON.parse(t).contents||''),()=>ttText('https://corsproxy.io/?url='+encodeURIComponent(f.url))];let last;for(const fn of tries){try{const a=ttParseRSS(await fn(),f);if(a.length)return a;last=Error('No items')}catch(e){last=e}}throw last||Error('Unavailable')}
function ttRenderMarketNews(){const list=$('#marketNewsList');if(!list)return;const rows=ttNewsItems.filter(x=>(ttNewsImpact==='all'||x.impact===ttNewsImpact)&&(ttNewsMarket==='all'||x.market===ttNewsMarket||x.market==='macro'));if(!rows.length){list.innerHTML='<div class="empty-state">No matching headlines for these filters right now.</div>';return}list.innerHTML=rows.slice(0,70).map(x=>{const d=ttNewsDate(x.pubDate);return`<article class="market-news-item"><div class="news-time"><strong>${esc(d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}))}</strong><small>${esc(ttTimeAgo(d))} ago</small></div><div class="news-market">${esc(x.market==='macro'?'MACRO':x.market)}</div><div class="news-body"><h3>${esc(x.title)}</h3>${x.description?`<p>${esc(x.description)}</p>`:''}</div><div class="news-impact-wrap"><span class="impact-pill ${esc(x.impact)}">${esc(x.impact.toUpperCase())}</span><span class="news-source">${esc(x.source)}</span></div><a class="news-open" href="${esc(x.link)}" target="_blank" rel="noopener noreferrer">Open ↗</a></article>`}).join('')}
async function ttLoadMarketNews(){if(ttNewsLoading)return;ttNewsLoading=true;const state=$('#marketNewsState'),upd=$('#marketNewsUpdated'),orb=$('#marketNewsOrb');if(state)state.textContent='Updating…';if(upd)upd.textContent='Checking FXStreet + market news';try{const rs=await Promise.allSettled(TT_NEWS_FEEDS.map(ttFetchFeed));let rows=[];rs.forEach(r=>{if(r.status==='fulfilled')rows.push(...r.value)});const failed=rs.filter(r=>r.status==='rejected').length,seen=new Set();rows=rows.filter(x=>{const k=(x.link!=='#'?x.link:x.title).toLowerCase();if(seen.has(k))return false;seen.add(k);return true}).sort((a,b)=>ttNewsDate(b.pubDate)-ttNewsDate(a.pubDate));if(rows.length)ttNewsItems=rows;ttRenderMarketNews();if(orb)orb.classList.toggle('error',!ttNewsItems.length);if(state)state.textContent=ttNewsItems.length?'Feed live':'Feed unavailable';if(upd)upd.textContent=ttNewsItems.length?`Updated ${new Date().toLocaleTimeString()} · ${ttNewsItems.length} headlines${failed?' · '+failed+'/'+TT_NEWS_FEEDS.length+' sources retrying':''}`:'Automatic retry in 20s'}catch(e){console.warn(e);if(orb)orb.classList.add('error');if(state)state.textContent='Feed unavailable';if(upd)upd.textContent='Automatic retry in 20s'}finally{ttNewsLoading=false;clearTimeout(ttNewsTimer);ttNewsTimer=setTimeout(ttLoadMarketNews,20000)}}
