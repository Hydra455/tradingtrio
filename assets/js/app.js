...
function aggregateComputed(rows,getKey){const m=new Map();rows.forEach(t=>{const key=getKey(t)||'Unspecified';if(!m.has(key))m.set(key,{key,count:0,wins:0,totalR:0});const x=m.get(key);x.count++;x.totalR+=num(t.r);if(num(t.r)>0)x.wins++});return[...m.values()].map(x=>({...x,wr:x.count?x.wins/x.count*100:0}))}
...
function card(t){const tags=(t.tags||[]).slice(0,3).map(x=>`<span class="tag">${esc(x)}</span>`).join('');const imageSrc=transientImageUrls.get(t.id)||t.image;
